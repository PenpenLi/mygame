...
-- initByData
-- 用网络数据初始化
function friendMgr.initByData(friendsData_, recvInvites_, sendInvites_)
	index = 1
	friends = {}
	recvInvites = {}
	sendInvites = {}

	if friendsData_~=nil then
		for i,v in ipairs(friendsData_) do
			friendMgr.addFriendByData(v, 1)
		end
	end
	if recvInvites_~=nil then
		for i,v in ipairs(recvInvites_) do
			friendMgr.addFriendByData(v, 2)
		end
	end
	if sendInvites_~=nil then
		for i,v in ipairs(sendInvites_) do
