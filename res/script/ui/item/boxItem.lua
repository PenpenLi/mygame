...
UI_boxItem = class("UI_boxItem", UI)

local boxList = 
{
	{24001, 24002, 24003, 24004},
	{24051, 24052, 24053, 24054}
}

local function isInBoxList(boxType, sid)
	for i, v in ipairs(boxList[boxType]) do
		if v==sid then
			return true
		end
	end
...
--init
function UI_boxItem:init(type_, useCallback_)
	-- data
	-- ===============================
	self.type = type_
	...
	if self.type == 1 then
		uiFrame:setTitle(hp.lang.getStrByID(3305))
	else
...
--
--onRemove
function UI_boxItem:onRemove()
	-- must release
	self.item1:release()
	self.item2:release()

	if self.haveUsed and self.useCallback~=nil then
		self.useCallback()
	end
...
-- onMsg
function UI_boxItem:onMsg(msg_, itemInfo_)
...
function UI_boxItem:updateItemList()
...
--
function UI_boxItem:pushLoadingItem(loadingNumOnce)
	...
	for i=self.loadingIndex+1, totalNum do
		...
		if isInBoxList(self.type, itemInfo.sid) then
