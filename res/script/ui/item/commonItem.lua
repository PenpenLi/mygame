...
UI_commonItem = class("UI_commonItem", UI)


--init
function UI_commonItem:init(itemSidType_, title_, checkFun_, useCallback_, addParam_)
	-- data
	-- ===============================
	self.itemSidType = itemSidType_
...
--
--onRemove
function UI_commonItem:onRemove()
	-- must release
	self.item1:release()
	self.item2:release()
...
-- onMsg
function UI_commonItem:onMsg(msg_, itemInfo_)
...
--
function UI_commonItem:pushLoadingItem(loadingNumOnce)
	...
	for i=self.loadingIndex+1, totalNum do
		...
		if itemType==self.itemSidType or itemInfo.sid==self.itemSidType then
