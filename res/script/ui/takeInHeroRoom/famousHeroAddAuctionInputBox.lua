...
UI_famousHeroAddAuctionInputBox = class("UI_famousHeroAddAuctionInputBox", UI)


--init
function UI_famousHeroAddAuctionInputBox:init(sid,price)
	-- data
	
	local heroInfo = hp.gameDataLoader.getInfoBySid("hero", sid)
	
	local wigetRoot = ccs.GUIReader:getInstance():widgetFromJsonFile(config.dirUI.root .. "famousHeroAddAuctionInputBox.json")
	...
	acutionTitle1:setString(hp.lang.getStrByID(6051))
	acutionTitle1:getChildByName("Label_acutionTitle2"):setString(price)
	acutionTitle1:getChildByName("Label_acutionTitle3"):setString(hp.lang.getStrByID(6018))
	
	--简介
	panelCont:getChildByName("Label_intro"):setString(string.format(hp.lang.getStrByID(6049)))
	panelCont:getChildByName("Label_intro_0"):setString(string.format(hp.lang.getStrByID(6050)))
	...
	local function onAuctionHeroHttpResponse(status, response, tag)
		if status==200 then
			...
			if data.result~=nil and data.result==0 then
				--加价成功
				msgbox = UI_msgBox.new(msgTips,hp.lang.getStrByID(6054),msgIs)
				self:addModalUI(msgbox)
	...
	local function btn_ok_callback(sender, eventType)
		...
		if eventType==TOUCH_EVENT_ENDED then
			...
			local function affirm()
				--用小于等于直接购买的价格 发送数据
				if oldPrice+curPri >= heroInfo.highPrice then 
					
					curPri = heroInfo.highPrice-oldPrice
					onBuyHero(curPri)
			...
			if curPri+oldPrice >= heroInfo.highPrice then
			--超过直接获取的价格
				msgbox = UI_msgBox.new(msgTips,string.format(hp.lang.getStrByID(6029),heroInfo.highPrice),msgIs,msgNo,twiceAffirm)
				self:addModalUI(msgbox)
			
			--加价大于等于1
			elseif curPri >= 1 then

				msgbox = UI_msgBox.new(hp.lang.getStrByID(1191),hp.lang.getStrByID(6053),msgIs,msgNo,affirm)
				self:addModalUI(msgbox)
				
			elseif curPri <= 0 then
				--低于最低加价
				--UI_msgBox.showCommonMsg(self, 1)
				require("ui/msgBox/msgBox")
					local msgBox = UI_msgBox.new(hp.lang.getStrByID(1191), hp.lang.getStrByID(6052))
				self:addModalUI(msgBox)
				return
			end
