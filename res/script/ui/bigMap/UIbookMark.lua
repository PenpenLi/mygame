...
local selectTab = {1, 0, 0, 0}
local image = {string.format("%s%s", config.dirUI.common, "kd_unChoose.png"), string.format("%s%s", config.dirUI.common, "kd_choose.png")}
local changeNum = {1, -1}
local textColor = {}
	...
	local function OnTabTouched(sender, eventType)
		...
		if eventType == TOUCH_EVENT_ENDED then
			...
			if tag == 4 then
				...
				for i = 1, 4 do
					selectTab[i] = st
					self.tab[i]:loadTexture(image[st + 1])
				...
				for i = 1, 3 do
					if i == tag then
						...
						self.tab[i]:loadTexture(image[selectTab[i] + 1])
				...
				if allSelect ~= selectTab[4] then
					...
					self.tab[4]:loadTexture(image[allSelect + 1])
...
function UI_bookMark:initCallBack()
	local function onEditTouched(sender, eventType)
		...
		if eventType == TOUCH_EVENT_ENDED then
			...
			ui = UI_addBookMark.new({k=info_:getPosK(),x=info_:getPosX(),y=info_:getPosY()}, 3, sender:getTag())
			self:addModalUI(ui)
		end
	end
	...
	local function onDeleteTouched(sender, eventType)
		...
		if eventType == TOUCH_EVENT_ENDED then
			...
			ui_ = UI_msgBox.new(hp.lang.getStrByID(5073), hp.lang.getStrByID(5074), hp.lang.getStrByID(1209),
	...
	local function onGoToTouched(sender, eventType)
		...
		if eventType == TOUCH_EVENT_ENDED then
			local info_ = player.getBookMarkByIndex(sender:getTag())
			game.curScene:gotoPosition(info_:getPosK(), {x=info_:getPosX(),y=info_:getPosY()})
			self:close()
		end
	end
...
function UI_bookMark:initUI()
	...
	textColor[1] = self.text[2]:getColor()
	textColor[2] = self.text[1]:getColor()
...
function UI_bookMark:listBookMark()
	...
	for i,v in ipairs(bookMarks_) do
		...
		if selectTab[type_] == 1 then
			...
			-- 坐标
			content_:getChildByName("Label_8027"):setString(string.format(hp.lang.getStrByID(1220), v:getPosK(), v:getPosX(), v:getPosY()))
