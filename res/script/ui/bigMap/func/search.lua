...
function UI_search:init()
	...
	local function OnConfirmTouched(sender, eventType)
		...
		if eventType == TOUCH_EVENT_ENDED then
			...
			game.curScene:gotoPosition(cc.p(tonumber(x), tonumber(y)), kName)
			self:close()
		end
	end
...
function UI_search:initUI()
	...
	self.textField[2].setString(tostring(curPosition.x))
	self.textField[3].setString(tostring(curPosition.y))
