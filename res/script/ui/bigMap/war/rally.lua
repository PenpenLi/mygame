...
--init
function UI_rally:init(position_, type_)
	-- data
	-- ===============================
	...
	self.type = type_
	if type_ == nil then
		self.type = globalData.MARCH_TYPE.RALLY_CITY
	end
...
function UI_rally:initCallBack()
	local function onSetTouched(sender, eventType)
		...
		if eventType == TOUCH_EVENT_ENDED then
			...
			cclog_(self.chooseID)
			UI_march.openMarchUI(self, self.position, self.type, {rallyTime=self.chooseID})
		end
	end
