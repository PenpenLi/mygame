--
-- ui/bigMap/source/mySource.lua
-- 点击资源弹出UI 
--===================================
require "ui/UI"
require "ui/frame/popFrame"

UI_mySource = class("UI_mySource", UI)

local imageList = {"gold.png", "silver.png", "food.png", "wood.png", "rock.png", "mine.png"}

--init
function UI_mySource:init(tileInfo_)
	-- data
	-- ===============================
...
function UI_mySource:initUI()
	self.wigetRoot = ccs.GUIReader:getInstance():widgetFromJsonFile(config.dirUI.root .. "mySource.json")
	...
	content:getChildByName("ImageView_13774"):loadTexture(config.dirUI.common..imageList[self.resourceInfo.growth+1])
...
function UI_mySource:tickUpdateInfo()
	local armyInfo_ = self.tileInfo.objInfo.armyInfo
	if armyInfo_ == nil then
		return
	end
	...
	local per = dt_ / self.totalTime * 100
	if num_ < 0 then
		num_ = 0
	end
...
function UI_mySource:heartbeat(dt)
...
function UI_mySource:getType()
	return globalData.ARMY_BELONG.ME
end

function UI_mySource:updateInfo()
	self:tickUpdateInfo()
end

function UI_mySource:onRemove()
