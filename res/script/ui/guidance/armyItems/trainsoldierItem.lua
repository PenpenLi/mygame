--
-- ui/guidance/armyItems/trainsoldierItem.lua
-- 训练
--===================================
local TrainsoldierItem = {}

local parent
local item
local iconUrl

-- 控件
local content
local text
local btn
local btnText
local checked
local progress
local progressBar
local timeLabel

local index
local priority
local isCheck
local haveBarracks
local canTrain
local isRun
local time
local total_time

local function checkedWarn(msgIsFunc)
	local msgbox = UI_msgBox.new(hp.lang.getStrByID(6034), 
		string.format(hp.lang.getStrByID(8167),hp.lang.getStrByID(8173)), 
		hp.lang.getStrByID(1209), hp.lang.getStrByID(2412), msgIsFunc)
	parent:addModalUI(msgbox)
end

local function checkedDone()
	local checkTbl = player.checkedPMTbl.getCheckedTbl()
	checkTbl[index] = isCheck
	checked:setVisible(isCheck==0)
	player.checkedPMTbl.setCheckedTbl(checkTbl)
	hp.msgCenter.sendMsg(hp.MSG.PM_CHECK_CHANGE, index)
end

local function onCheckTouch(sender, eventType)
	hp.uiHelper.btnImgTouched(sender, eventType)
	if eventType == TOUCH_EVENT_ENDED then
		if isCheck == 0 then
			local function msgIsFunc()
				isCheck = 1
				checkedDone()
			end
			checkedWarn(msgIsFunc)
		else
			isCheck = 0
			checkedDone()
		end
	end
end

local function onBtnTouch(sender, eventType)
	hp.uiHelper.btnImgTouched(sender, eventType)
	if eventType == TOUCH_EVENT_ENDED then
		if haveBarracks then
			if isRun then
				parent:addUI(UI_speedItem.new(cdBox.CDTYPE.BRANCH))
			elseif canTrain then
				player.buildingMgr.getBuildingObjBySid(1009):onClicked()
			end
		else
			parent:closeAll()
			if game.curScene.mapLevel == 3 then
				require("scene/cityMap")
				local map = cityMap.new()
				map:enter()
			end
			local block = game.curScene:getBlock(1)
			block:Scroll2Here(0.5)
			block:addGuide()
		end
	end
end

-- 设置外观
local function setExterior()
	local iconBg = content:getChildByName("Image_iconBg")
	iconBg:getChildByName("Image_icon"):loadTexture(config.dirUI.common .. iconUrl)

	isCheck = player.checkedPMTbl.getCheckedTbl()[index]
	checked:setVisible(isCheck==0)
end

-- 设置监听
local function setListener()
	local checkbox = content:getChildByName("Image_checkBox")
	checkbox:addTouchEventListener(onCheckTouch)
	local btn = content:getChildByName("Image_btnGet")
	btn:addTouchEventListener(onBtnTouch)
end

-- 初始化
function TrainsoldierItem.init(parent_, item_, index_)
	parent = parent_
	item = item_:clone()
	item:retain()
	index = index_

	iconUrl = "cd_icon_remedy.png"

	content = item:getChildByName("Panel_content")
	text = content:getChildByName("Label_txt")
	btn = content:getChildByName("Image_btnGet")
	btnText = content:getChildByName("Label_info")
	checked = content:getChildByName("Image_checkBox"):getChildByName("Image_checked")
	progress = item:getChildByName("Panel_frame"):getChildByName("Progress_bg")
	progressBar = progress:getChildByName("Progress_bar")
	timeLabel = content:getChildByName("Label_time")

	priority = 0
	isCheck = 0
	haveBarracks = false
	canTrain = true
	isRun = false
	time = 0
	total_time = 0

	setExterior()
	setListener()
	TrainsoldierItem.setData()
end

-- 播放文字动画
function TrainsoldierItem.playLabelAni()
	text:stopAllActions()
	setLabelAni(text, hp.lang.getStrByID(3104), cdBox.getDescInfo(cdBox.CDTYPE.BRANCH))
end

-- 停止文字动画
function TrainsoldierItem.stopLabelAni()
	text:stopAllActions()
end

-- 设置数据
function TrainsoldierItem.setData()
	timeLabel:setString("")
	text:setOpacity(255)
	text:stopAllActions()
	
	local barracksBuild = player.buildingMgr.getBuildingNumBySid(1009)
	haveBarracks = barracksBuild ~= nil and barracksBuild > 0
	if haveBarracks then
		isRun = cdBox.getCDInfo(cdBox.CDTYPE.BRANCH).cd > 0
		if isRun then
			priority = 2
			btnText:setString(hp.lang.getStrByID(8138))
			time = cdBox.getCDInfo(cdBox.CDTYPE.BRANCH).cd
			total_time = cdBox.getCDInfo(cdBox.CDTYPE.BRANCH).total_cd
			timeLabel:setString(hp.datetime.strTime(time))
			progressBar:setPercent((1 - time / total_time) * 100)
			TrainsoldierItem.playLabelAni()
		else
			local soldierInfo = game.data.main[player.buildingMgr.getBuildingMaxLvBySid(1001)]
			local soldierMax = soldierInfo.soldierMax * soldierInfo.troopMax
			canTrain = player.soldierManager.getTotalArmy():getSoldierTotalNumber() < soldierMax
			if canTrain then
				priority = 1
				text:setString(hp.lang.getStrByID(8153))
				btnText:setString(hp.lang.getStrByID(8156))
			else
				priority = 4
				text:setString(hp.lang.getStrByID(8155))
			end
		end
	else
		priority = 1
		text:setString(hp.lang.getStrByID(8154))
		btnText:setString(hp.lang.getStrByID(8157))
	end
	btn:setVisible(canTrain)
	btn:setTouchEnabled(canTrain)
	btnText:setVisible(canTrain)
	progress:setVisible(isRun)
end

function TrainsoldierItem.heartbeat(dt)
	if isRun then
		time = cdBox.getCDInfo(cdBox.CDTYPE.BRANCH).cd
		timeLabel:setString(hp.datetime.strTime(time))
		progressBar:setPercent((1 - time / total_time) * 100)
		if time <= 0 then
			TrainsoldierItem.setData()
		end
	end
end

function TrainsoldierItem.getItem()
	return item
end

function TrainsoldierItem.getPriority()
	return priority
end

function TrainsoldierItem.getCheckIndex()
	return index
end

function TrainsoldierItem.setBtnLight()
	local light = inLight(btn:getVirtualRenderer(), 1)
	btn:addChild(light)
end

function TrainsoldierItem.removeBtnLight()
	btn:removeAllChildren()
end

function TrainsoldierItem.onRemove()
	item:release()
end

return TrainsoldierItem